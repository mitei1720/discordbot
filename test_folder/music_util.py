...
class Music_data:
    def __init__(self,name:str,artist:str,genre:str,diff:str,level:int,lconst:float):
        self.name = name
        self.artist = artist
        self.genre = genre
        self.diff = diff
...
def randompick(genre:str = None,level:float = None,level_from:float = None,level_to:float =None,diff:str=None)-> Tuple[Union[Music_data,None],Union[str,None]]:
    """
    ...
    with du.DBwrapper(DATABASE) as db:
        ...
        if imax != 0:
            ...
            return Music_data(rr["meta_title"],rr["meta_artist"],rr["meta_genre"],rr["diff"],rr["level"],rr["const"]), None
        else:
            return None,"No such songs!!"



def searchpick(title:str = None, genre:str = None,level:float = None,level_from:float = None,level_to:float =None,diff:str=None)-> Tuple[list,Union[str,None]]:
    """
    全曲データから指定条件で曲を一曲ランダムピック
    ------------------------------------------
    引数を指定しない場合は絞り込まない。
    ...
    with du.DBwrapper(DATABASE) as db:
        ...
        for row in db.get("all_music",dat):
            song_list.append(Music_data(row["meta_title"],row["meta_artist"],row["meta_genre"],row["diff"],row["level"],row["const"]))
        ...
        if imax >= 6:
            ...
            return song_list,"Select from here."
...
print(get_score_goals("4997052439ec7c0a","m",17.30))
